...
export default async function AdminClassesTermSelectPage({
  params,
}: {
  ...
  return (
    <DashboardShell name={user.name} role="ADMIN">
      <Link href="/admin/classes" className="text-sm text-[var(--color-muted)] hover:underline">
        ← الرجوع لاختيار السنة الدراسية
      </Link>

      <div className="mt-4 mb-8">
        <h1 className="font-display text-2xl font-extrabold text-[var(--color-primary-dark)]">
          فصول المدرسة - السنة الدراسية {academicYear}
        </h1>
        <p className="text-sm text-[var(--color-muted)] mt-1">
          اختر الفصل الدراسي (الترم) لعرض درجات وغياب فصول المدرسة.
        </p>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 max-w-2xl mx-auto">
        {TERM_OPTIONS.map((term) => (
          <Link
            key={term}
            href={`/admin/classes/year/${yearParam}/term/${term}`}
          ...
          >
            ...
            <span className="text-xs text-[var(--color-muted)]">
              عرض المواد وكشوف الطلاب والدرجات المسجلة في {TERM_LABELS[term]}.
            </span>
          </Link>
        ))}
      </div>
