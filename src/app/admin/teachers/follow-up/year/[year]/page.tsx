...
export default async function AdminTeacherFollowUpTermSelectPage({
  params,
}: {
  ...
  return (
    <DashboardShell name={user.name} role="ADMIN">
      <Link href="/admin/teachers/follow-up" className="text-sm text-[var(--color-muted)] hover:underline">
        ← الرجوع لاختيار السنة الدراسية
      </Link>

      <div className="mt-4 mb-8">
        <h1 className="font-display text-2xl font-extrabold text-[var(--color-primary-dark)]">
          متابعة المدرسين - السنة الدراسية {academicYear}
        </h1>
        <p className="text-sm text-[var(--color-muted)] mt-1">
          اختر الفصل الدراسي (الترم) لمتابعة وعرض سجلات المدرسين.
        </p>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 max-w-2xl mx-auto">
        {TERM_OPTIONS.map((term) => (
          <Link
            key={term}
            href={`/admin/teachers/follow-up/year/${yearParam}/term/${term}`}
          ...
          >
            ...
            <span className="text-xs text-[var(--color-muted)]">
              عرض سجلات المدرسين والمواد المسندة لهم في {TERM_LABELS[term]}.
            </span>
          </Link>
        ))}
      </div>
